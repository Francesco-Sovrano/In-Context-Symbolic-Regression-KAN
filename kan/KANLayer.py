...
class KANLayer(nn.Module):
    ...
    def __init__(self, input_dim=3, output_dim=2, grid_min=-2., grid_max=2., num_grids=5, k=3, noise_scale=0.5, scale_base_mu=0.0, scale_base_sigma=1.0, scale_sp=1.0, base_fun=torch.nn.SiLU(), grid_eps=0.02, sp_trainable=True, sb_trainable=True, save_plot_data = True, device='cpu', sparse_init=False, **args):
        super(KANLayer, self).__init__()
        # size 
        self.out_dim = out_dim = output_dim
        ...
        self.register_buffer("grid", grid)  # buffer instead of Parameter
        mask0 = sparse_mask(in_dim, out_dim) if sparse_init else torch.ones(in_dim, out_dim)
    ...
    def update_grid_from_samples(self, x, mode='sample'):
        ...
        with torch.no_grad():
            self.grid.copy_(extend_grid(grid, k_extend=self.k))
            self.coef.copy_(curve2coef(x_pos, y_eval, self.grid, self.k))
    ...
    def get_subset(self, in_id, out_id):
        ...
        with torch.no_grad():
            ...
            spb.scale_base.copy_(self.scale_base[in_id][:, out_id])
            spb.scale_sp.copy_(self.scale_sp[in_id][:, out_id])
            spb.mask.copy_(self.mask[in_id][:, out_id])
    ...
    def swap(self, i1, i2, mode='in'):
        ...
        with torch.no_grad():
            ...
            if mode == 'in':
                swap_(self.grid, i1, i2, mode='in')
            swap_(self.coef, i1, i2, mode=mode)
            swap_(self.scale_base, i1, i2, mode=mode)
            swap_(self.scale_sp, i1, i2, mode=mode)
            swap_(self.mask, i1, i2, mode=mode)
