...
class MultKAN(nn.Module):
	def __init__(self, width=None, grid=3, k=3, mult_arity = 2, noise_scale=0.3, scale_base_mu=0.0, scale_base_sigma=1.0, base_fun='silu', affine_trainable=False, grid_eps=0.02, grid_range=[-1, 1], sp_trainable=True, sb_trainable=True, seed=1, save_act=True, sparse_init=False, auto_save=True, first_init=True, ckpt_path='./model', state_id=0, round=0, device='cpu', atom_names=None, numeric_atom_configs=None, chain_nodes=0, chain_types=None):
		...
		self.use_gated_layer = self.atom_names or self.numeric_atom_configs
	...
	def forward(self, x, singularity_avoiding=False, y_th=10.):
		...
		# --- legacy caches (needed by old_fix_symbolic + some viz utilities) ---
		self.acts_premult = []
		self.spline_preacts = []
		...
		for l in range(self.depth):
			...
			if self.save_act:
				...
				# --- legacy caches for old_fix_symbolic / analysis ---
	...
	def old_fix_symbolic(self, l, i, j, fun_name, fit_params_bool=True, a_range=(-10, 10), b_range=(-10, 10), verbose=True, random=False, log_history=True):
		'''
		...
		if not fit_params_bool:
			self.symbolic_fun[l].old_fix_symbolic(i, j, fun_name, verbose=verbose, random=random)
			r2 = None
		else:
			x = self.acts[l][:, i]
			...
			r2 = self.symbolic_fun[l].old_fix_symbolic(i, j, fun_name, x, y, a_range=a_range, b_range=b_range, verbose=verbose)
			if mask[i,j] == 0:
				r2 = - 1e8
		self.set_mode(l, i, j, mode="s")
		
		if log_history:
			self.log_history('old_fix_symbolic')
		return r2
	...
	def suggest_symbolic(self, l, i, j, a_range=(-10, 10), b_range=(-10, 10), lib=None, topk=5, verbose=True, r2_loss_fun=lambda x: np.log2(1+1e-5-x), c_loss_fun=lambda x: x, weight_simple = 0.8):
		...
		for (name, content) in symbolic_lib.items():
			r2 = self.old_fix_symbolic(l, i, j, name, a_range=a_range, b_range=b_range, verbose=False, log_history=False)
	...
	def baseline_symbolic_regression(self, a_range=(-10, 10), b_range=(-10, 10), lib=None, verbose=1, weight_simple = 0.8, r2_threshold=0.0):
		'''
			...
			for i in range(self.width_in[l]):
				for j in range(self.width_out[l + 1]):
					if self.symbolic_fun[l].mask[j, i] > 0. and self.act_fun[l].mask[i][j] == 0.:
						...
						self.old_fix_symbolic(l, i, j, '0', verbose=verbose > 1, log_history=False)
						...
						if r2 >= r2_threshold:
							self.old_fix_symbolic(l, i, j, name, verbose=verbose > 1, log_history=False)
							if verbose >= 1:
	...
	def expand_depth(self):
		...
		if not self.use_gated_layer:
			layer = KANLayer(dim_out, dim_out, num=self.grid).to(self.device)
		else:
		...
		def _expand(layer_id, n_added_nodes, sum_bool=True, mult_arity=2, added_dim='out'):
			...
			if sum_bool:
				...
				if added_dim == 'out':
					...
					if not self.use_gated_layer:
						self.act_fun[l] = KANLayer(in_dim, out_dim + n_added_nodes, num=self.grid).to(self.device)
					else:
				...
				if added_dim == 'in':
					...
					self.symbolic_fun[l] = new
					if not self.use_gated_layer:
						self.act_fun[l] = KANLayer(in_dim + n_added_nodes, out_dim, num=self.grid).to(self.device)
					else:
				...
				if added_dim == 'out':
					...
					self.symbolic_fun[l] = new
					if not self.use_gated_layer:
						self.act_fun[l] = KANLayer(in_dim, out_dim + n_added_subnodes, num=self.grid).to(self.device)
					else:
